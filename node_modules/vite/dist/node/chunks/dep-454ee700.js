...
function createTransformContext(root, { filename = '', prefixIdentifiers = false, hoistStatic = false, cacheHandlers = false, nodeTransforms = [], directiveTransforms = {}, transformHoist = null, isBuiltInComponent = NOOP$1, isCustomElement = NOOP$1, expressionPlugins = [], scopeId = null, ssr = false, ssrCssVars = ``, bindingMetadata = EMPTY_OBJ$1, inline = false, isTS = false, onError = defaultOnError }) {
...
function createRootCodegen(root, context) {
    ...
    if (children.length === 1) {
        ...
        let patchFlagText = PatchFlagNames$1[64 /* STABLE_FRAGMENT */];
        // check if the fragment actually contains a single valid child with
        // the rest being comments
        if ((process.env.NODE_ENV !== 'production') &&
            children.filter(c => c.type !== 3 /* COMMENT */).length === 1) {
            patchFlag |= 2048 /* DEV_ROOT_FRAGMENT */;
            patchFlagText += `, ${PatchFlagNames$1[2048 /* DEV_ROOT_FRAGMENT */]}`;
        }
...
function genNodeListAsArray(nodes, context) {
    const multilines = nodes.length > 3 ||
        (((process.env.NODE_ENV !== 'production')) && nodes.some(n => isArray$1(n) || !isText$1(n)));
    context.push(`[`);
    ...
    if (needFragmentWrapper) {
        if (children.length === 1 && firstChild.type === 11 /* FOR */) {
            ...
            return createVNodeCall(context, helper(FRAGMENT), createObjectExpression([keyProperty]), children, 64 /* STABLE_FRAGMENT */ +
                ((process.env.NODE_ENV !== 'production')
                    ? ` /* ${PatchFlagNames$1[64 /* STABLE_FRAGMENT */]} */`
...
const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
    ...
    return processFor(node, dir, context, forNode => {
        ...
        forNode.codegenNode = createVNodeCall(context, helper(FRAGMENT), undefined, renderExp, fragmentFlag +
            ((process.env.NODE_ENV !== 'production') ? ` /* ${PatchFlagNames$1[fragmentFlag]} */` : ``), undefined, undefined, true /* isBlock */, !isStableFragment /* disableTracking */, node.loc);
        return () => {
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
    ...
    createSimpleExpression(slotFlag + ((process.env.NODE_ENV !== 'production') ? ` /* ${slotFlagsText$1[slotFlag]} */` : ``), false))), loc);
        ...
        if (patchFlag !== 0) {
            if ((process.env.NODE_ENV !== 'production')) {
                if (patchFlag < 0) {
                    ...
                    vnodePatchFlag = patchFlag + ` /* ${PatchFlagNames$1[patchFlag]} */`;
                }
                else {
                    // bitwise flags
                    const flagNames = Object.keys(PatchFlagNames$1)
                        .map(Number)
                        .filter(n => n > 0 && patchFlag & n)
                        .map(n => PatchFlagNames$1[n])
                        .join(`, `);
...
const transformOn = (dir, node, context, augmentor) => {
    ...
    if (arg.type === 4 /* SIMPLE_EXPRESSION */) {
        if (arg.isStatic) {
            ...
            eventName = createSimpleExpression(toHandlerKey$1(camelize$2(rawName)), true, arg.loc);
        }
        else {
            // #2388
                ...
                if (isText(child) || child.type === 8 /* COMPOUND_EXPRESSION */) {
                    ...
                    if (!context.ssr &&
                        ...
                        callArgs.push(1 /* TEXT */ +
                            ((process.env.NODE_ENV !== 'production') ? ` /* ${PatchFlagNames$1[1 /* TEXT */]} */` : ``));
                    }
                    children[i] = {
                        type: 12 /* TEXT_CALL */,
                        content: child,
...
function baseCompile(template, options = {}) {
    ...
    transform(ast, extend$1({}, options, {
        ...
        directiveTransforms: extend$1({}, directiveTransforms, options.directiveTransforms || {} // user transforms
        )
    }));
